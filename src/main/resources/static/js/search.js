const url_search = "http://localhost:8081/api/posts/conditions";
let headerSearch = new Headers();
headerSearch.append("Content-Type", "application/json");


const btn_menu_search = document.querySelector('.menu-search-item');
const form_search_posts = document.querySelector('.search-posts-box');
console.log([form_search_posts]);
const btn_create_post = document.querySelector('.create-post');
const feeds_result = document.querySelector('.feeds');
const btn_menu_home = document.querySelector('.menu-item-home');
const search_content = document.querySelector('.search-content');
const btn_up_down_search_content = document.querySelector('.search-posts-box .search-header img');
let height_search_box = 974;

console.log(search_content, btn_up_down_search_content);

btn_up_down_search_content.addEventListener('click', async()=>{
    search_content.classList.toggle('minimize');
    if(search_content.classList.value.includes('minimize')){
        await minimizeSearchBox();
    } else{
        await maxSizeSearchBox();
    }
})

function minimizeSearchBox(){
    let size = height_search_box;
    const minimize = setInterval(() => {
        size-=10;
        if(size<=95) size=95;
        form_search_posts.style.height = `${size}px`;
        if(size==95) clearInterval(minimize);
    }, 3);
}

function maxSizeSearchBox(){
    let size = 95;
    const maxSize = setInterval(() => {
        size+=10;
        if(size>=974) size=974;
        form_search_posts.style.height = `${size}px`;
        if(size==974) clearInterval(maxSize);
    }, 3);
}

// console.log(btn_menu_search);
...
if(btn_menu_home!=null){
    btn_menu_home.addEventListener('click', () => {
        form_search_posts.style.display = 'none';
        // feeds_result.innerHTML = '';
        btn_create_post.style.display = 'flex';    
    })

}


const btn_search_posts = form_search_posts.querySelector('.btn-search-posts');
console.log([btn_search_posts]);

// select utilities
const input_utilities_search = form_search_posts.querySelectorAll('.input .util-item');
input_utilities_search.forEach(e => {
...
// const input_select_search = form_search_posts.querySelectorAll('select');
// input_select_search.forEach(e => {
//     console.log(e.options[e.selectedIndex].value);
// })

btn_search_posts.addEventListener('click', async () => {
    const input_select_search = form_search_posts.querySelectorAll('select');
    const input_text_search = form_search_posts.querySelectorAll('input[type="text"]');
    const input_check_box_search = form_search_posts.querySelectorAll('input[type="checkbox"]');

    const postCondition = { };

    input_utilities_search.forEach(e => {
        if(e.classList.contains('active')){
            postCondition[e.attributes[1].value] = 1;
        }else{
            postCondition[e.attributes[1].value] = 0;
        }
    })
    ...
    input_select_search.forEach(e => {
        // if(e.name == "firstSize" || e.name == "lastSize"){
        //     postCondition[e.name] = e.options[e.selectedIndex].value;
        // } else if(e.name == "firstRentPrice" || e.name == "lastRentPrice"){
        //     postCondition[e.name] = e.options[e.selectedIndex].value;
        // } else{
    ...
    feeds_result.style.display = 'block';
    search_content.classList.add('minimize');
    minimizeSearchBox(); //minimize search box


    const posts = await posts_result.map(e => {
        e.post.score = e.score;
        return e.post;
    });
    ...
    await posts.forEach(e => {
        ...
        <div class="feed" id="p${e.id}">
            <div class="head">
                ...
                <span class="edit">
                    <p >Độ phù hợp: 
                    <span class="percent_compatible" style="color:var(--color-success); margin-right:5px; font-size: 1rem;" >${e.score}%</span>
                     </p>
                </span>
            </div>
